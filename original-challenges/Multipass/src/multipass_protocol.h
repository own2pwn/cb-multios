 ...
 * ALL operations *
 server -> client (cgc_PKT_TYPE = INIT or AUTH or OPS or FIN)
 ...
 * PURCHASE operation *
 cgc_OP_CODE = PURCHASE

 client -> server (cgc_PKT_TYPE = AUTH)
 ...
 server -> client (cgc_PKT_TYPE = AUTH)
 - server sends Common Packet Header
 -- status will be set to a value from the cgc_STATUS enum.
 ...
 client -> server (cgc_PKT_TYPE = OPS)
 ...
 -- vendor_type can be any value from cgc_VENDOR_TYPE enum
 server -> client (cgc_PKT_TYPE = OPS)
 - server sends Common Packet Header
 -- status will be set to a value from the cgc_STATUS enum.
 -- transaction_id will be set to the ID of the current transaction.
 -- card_id and auth_code will be the same as those provided to the server by the client.
 client -> server (cgc_PKT_TYPE = FIN)
 - client sends Common Packet Header
 -- card_id and auth_code must be from same card used in AUTH and OPS.
 -- transaction_id must be that provided by server during AUTH
 -- status is unused (recommend to set to 0)
 server -> client (cgc_PKT_TYPE = FIN)
 - server sends Common Packet Header
 -- status will be set to a value from the cgc_STATUS enum.
 -- transaction_id will be set to the ID of the current transaction.
 -- card_id and auth_code will be the same as those provided to the server by the client.
 ...
 * HISTORY operation *
 ...
 -- transaction_id must be that provided by server during AUTH
 -- status is unused (recommend to set to 0)
 -- count tells the server to cgc_send up to that many of the most recent transactions
 ...
 -- for all transactions, cgc_send Transaction Record followed by:
 --- if transaction cgc_OP_CODE==PURCHASE, cgc_send Purchase Data and associated Vendor Data
 --- if transaction cgc_OP_CODE==BALANCE, cgc_send Balance Data
 --- if transaction cgc_OP_CODE==ISSUE, cgc_send Issue Data
 --- if transaction cgc_OP_CODE==RECHARGE, cgc_send Recharge Data and associated Vendor Data
 --- if transaction cgc_OP_CODE==REFUND, cgc_send Refund Data
 --- if transaction cgc_OP_CODE==HISTORY, cgc_send History Data
 client -> server (cgc_PKT_TYPE = FIN)
 - client sends Common Packet Header
 -- card_id and auth_code must be from same card used in AUTH and OPS.
 -- transaction_id must be that provided by server during AUTH
 ...
 * ISSUE operation *
 ...
 client -> server (cgc_PKT_TYPE = INIT)
 - client sends Common Packet Header
 -- card_id and auth_code are unused (recommend to set to 0).
 -- status and transaction_id are unused (recommend to set to 0).
 server -> client (cgc_PKT_TYPE = INIT)
 - server sends Common Packet Header
...
** Errors **
 Any errors in the server will cause the server to cgc_send to the client:
 ...
 3. Then the server will cgc_exit.
	...
	* Error Messages *
	...
	ERRNO_MP_INVALID_OPCODE	-> "Invalid cgc_OP_CODE."
	ERRNO_MP_INVALID_P_TYPE	-> "Invalid cgc_PKT_TYPE."
	ERRNO_MP_INVALID_V_TYPE	-> "Invalid cgc_VENDOR_TYPE."
...
typedef struct packet_head {
	...
	cgc_STATUS status : 8;			// 1 byte
	cgc_uint32_t transaction_id;	// 4 bytes
} __attribute__((__packed__)) cgc_packet_head_t;

/* Error Data */
typedef struct packet_data_error {
	cgc_uint32_t length;
	char msg[64];
} __attribute__((__packed__)) cgc_packet_data_error_t;
...
typedef struct vendor {
	...
	cgc_uint8_t vendor_location_sz;
	char vendor_location[];
} __attribute__((__packed__)) cgc_vendor_t;

/* Issue Data */
typedef struct packet_data_issue {
	cgc_uint32_t amount;
} __attribute__((__packed__)) cgc_packet_data_issue_t;

/* Balance Data */
typedef struct packet_data_balance {
	cgc_uint32_t balance;
} __attribute__((__packed__)) cgc_packet_data_balance_t;

/* History Data */
typedef struct packet_data_history {
	cgc_uint32_t count;
} __attribute__((__packed__)) cgc_packet_data_history_t;
...
typedef struct packet_data_purchase {
	...
	cgc_uint32_t cost;
	cgc_vendor_t v;
} __attribute__((__packed__)) cgc_packet_data_purchase_t;

/* Recharge Data */
typedef struct packet_data_recharge {
	cgc_uint32_t amount;
	cgc_vendor_t v;
} __attribute__((__packed__)) cgc_packet_data_recharge_t;

/* Refund Data */
typedef struct packet_data_refund {
	cgc_uint32_t purchase_id;
	cgc_uint32_t transaction_id;
} __attribute__((__packed__)) cgc_packet_data_refund_t;
...
typedef struct packet_data_transaction {
    ...
    cgc_uint32_t card_id;
    cgc_uint32_t id;
} __attribute__((__packed__)) cgc_packet_data_transaction_t;
