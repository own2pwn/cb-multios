...
{
#ifdef PATCHED_1
  if ( (((cgc_uint32_t)addr & PAGE_MASK) == 0) || (((cgc_uint32_t)addr & PAGE_MASK) == (MAGIC_PAGE & PAGE_MASK) ) )
#else
  if ( (((cgc_uint32_t)addr & PAGE_MASK) == 0) || (((cgc_uint32_t)addr & PAGE_MASK) == (0 & PAGE_MASK) ) )
#endif
  {
    return (0);
  }
...
int cgc_HASH(const cgc_uint8_t* buf, cgc_size_t size, cgc_uint8_t* hashOutput)
{
  ...
  while (temp < (buf + size))
  {
    ...
    if (!cgc_isValidAddr(temp))
    {
      ...
      temp = (cgc_uint8_t*)((cgc_uint32_t)temp & PAGE_MASK);
      continue;
    }
...
int cgc_INIT(cgc_uint8_t* hash, char c)
{
  if (hash == NULL)
  {
    return (-1);
...
cgc_uint32_t cgc_hexStrToUint32(const char* str)
{
  int i = 0;
  cgc_uint32_t ret = 0;
  cgc_uint32_t temp = 0;
...
void cgc_hashToHexStr(const cgc_uint8_t* hash, char* str)
{
  int i = 0;
  char c = 0;
  for (i = 0; i < HASH_SIZE; i++)
