...
typedef struct cgc_instruction {
	char text[128];
...
} cgc_recipe, *cgc_precipe;

/// This is done to force ordering
typedef struct cgc_global {
	cgc_pantry pantry_g[PANTRY_COUNT];
	cgc_recipe recipe_list_g[RECIPE_COUNT];
} cgc_global, *cgc_pglobal;
...
cgc_pinstruction cgc_instruction_prompt()
{
	cgc_pinstruction pins = NULL;

	pins = (cgc_pinstruction)cgc_malloc( sizeof( cgc_instruction) );

	if ( pins == NULL ) {
		return pins;
	}

	cgc_bzero( pins, sizeof(cgc_instruction));
...
cgc_pingredient cgc_ingredient_prompt( )
{
	cgc_pingredient ping = NULL;
	char data[6];

	ping = ( cgc_pingredient )cgc_malloc( sizeof( cgc_ingredient) );

	if ( ping == NULL ) {
		return ping;
	}

	cgc_bzero( ping, sizeof(cgc_ingredient) );

	cgc_printf("Enter cgc_ingredient name or a \"done\" to quit.\n");
	cgc_printf(": ");
...
void cgc_link_recipe_ingredient( cgc_precipe pr, cgc_pingredient ping )
{
...
int cgc_check_ingredient_exists( cgc_pingredient head, cgc_pingredient new)
{
...
void cgc_link_recipe_instruction( cgc_precipe pr, cgc_pinstruction pins )
{
	cgc_pinstruction walker = NULL;
...
void cgc_add_recipe( void )
{
	...
	cgc_bzero( pr, sizeof(cgc_recipe) );

	cgc_printf("Enter cgc_recipe name: ");

	if ( cgc_read_string( pr->name, 15 ) == 0 ) {
		cgc_printf("[ERROR] Invalid cgc_recipe name\n");
		return;
	}

	t = index;

	/// Ensure that the cgc_recipe name does not already exist
...
void cgc_print_recipe( int index )
{
	...
	if ( g.recipe_list_g[index].cookie == 0 ) {
		cgc_printf("[ERROR] Empty cgc_recipe: $d\n", index+1);
		return;
	}
...
void cgc_remove_recipe( )
{
	...
	if ( g.recipe_list_g[choice-1].cookie == 0 ) {
		cgc_printf("[ERROR] Empty cgc_recipe\n");
		return;
	}

	cgc_bzero( &(g.recipe_list_g[choice-1]), sizeof(cgc_recipe) );
...
{
	cgc_bzero( &g.pantry_g, sizeof(cgc_pantry) * PANTRY_COUNT);
	cgc_bzero( &g.recipe_list_g, sizeof(cgc_recipe) * RECIPE_COUNT);
...
void cgc_link_pantry_ingredient( cgc_ppantry pp, cgc_pingredient ping )
{
...
void cgc_print_single_pantry( size_t index )
{
	...
	if ( g.pantry_g[index].cookie == 0 ) {
		cgc_printf("[ERROR] Empty cgc_pantry: $d\n", index+1);
		return;
	}
...
void cgc_create_pantry( void )
{
	...
	cgc_bzero( pp, sizeof(cgc_pantry) );

	cgc_printf("Enter cgc_pantry name: ");

	if ( cgc_read_string( pp->name, 15 ) == 0 ) {
		cgc_printf("[ERROR] Invalid cgc_pantry name\n");
		return;
	}

	t = index;

	/// Ensure that the cgc_pantry name does not already exist
...
void cgc_delete_pantry( void )
{
	...
	if ( g.pantry_g[choice-1].cookie == 0 ) {
		cgc_printf("[ERROR] Empty cgc_pantry\n");
		return;
	}

	cgc_bzero( &(g.pantry_g[choice-1]), sizeof(cgc_pantry) );
...
void cgc_update_pantry( void )
{
	...
	if ( ing_walker == NULL ) {
		cgc_printf("This cgc_pantry does not have $s\n", data );
		return;
	}
...
void cgc_query_recipes( void )
{
	...
	cgc_pingredient riw = NULL;

	/// Pantry cgc_ingredient walker
	cgc_pingredient piw = NULL;

	cgc_printf("Select a cgc_pantry:\n");
	...
	/// Loop through each cgc_recipe.
	/// For each cgc_ingredient loop through the grocery list of the cgc_pantry
	/// If the cgc_ingredient is found, check the amount.
	/// If there is enough then check the next cgc_ingredient
	///	If not then move on to the next cgc_recipe.
	for ( index = 0; index < RECIPE_COUNT; index ++ ) {
		if ( g.recipe_list_g[index].cookie == 0 ) {
			continue;
		}
...
void cgc_print_cheapest( void )
{
	...
	if ( cheapest_index == 0 ) {
		cgc_printf("No cheapest cgc_recipe found\n");
	} else {
...
void cgc_print_expensivest( void )
{
	...
	if ( expensivest_index == 0 ) {
		cgc_printf("The most expensive cgc_recipe was not found\n");
	} else {
