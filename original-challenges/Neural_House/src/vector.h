...
class cgc_vector
{
  public:
    cgc_vector();
    cgc_vector(size_t sz);
    cgc_vector(const cgc_vector<T> &vec);
    ~cgc_vector();

    bool cgc_empty() const;
    size_t cgc_size() const;
    size_t cgc_capacity() const;
    T& cgc_front();
    T& cgc_back();
    void cgc_push_back(const T& val);
    void cgc_pop_back();
    void cgc_reserve(size_t cap);
...
template<class T>
cgc_vector<T>::cgc_vector(size_t sz)
{
...
template<class T>
void cgc_vector<T>::cgc_reserve(size_t cap)
{
  T* d = new T[cap];
  for (unsigned int i = 0; i < m_size; ++i)
    d[i] = m_data[i];
...
template<class T>
size_t cgc_vector<T>::cgc_size() const
{
  return m_size;
}

template<class T>
size_t cgc_vector<T>::cgc_capacity() const
{
  return m_capacity;
}
